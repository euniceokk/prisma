...
server <- function(input, output, session) {
  ...
  output$st_selector_ui <- renderUI({
    k <- safe_int(input$k_st)
    if (k <= 1) {
      # Hidden input so conditionalPanel for s=1 always evaluates correctly
      tags$input(type = "hidden", id = "active_st", value = "1")
    } else {
  ...
  sci_criteria_st <- function(s) {
    c(paste0("n \u2265 ", safe_int(input[[paste0("n_threshold_st", s)]])),
      trim_lines(input[[paste0("sci_custom_st", s)]]))
  }
